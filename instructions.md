...
The DAO governance portal is the primary way that users interact with the Hyperware DAO.
The Hyperware DAO is an OpenZeppelin DAO with some customization.
The governance portal allows standard interactions and views of the state of the DAO.

### Relevant code

src/dao/HyperwareGovernor.sol
```
    ...
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        __Governor_init("HyperwareGovernor");
    ...
    function proposalThreshold()
        ...
        returns (uint256)
    {
        return super.proposalThreshold() * 100;
    }
...
src/dao/TimelockControllerUpgradeableWithInit.sol
```
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.27;
...
register-ui connect wallet example
package.json
```
{
  "name": "hyperdrive-register-ui",
  "version": "0.1.0",
  "private": true,
...
```
example page implementing connect button: MintDotOsName.tsx
```
...
Splash screen queries the backend with `Ready` requests, timeout 2s, until the backend replies with a `Ready`.
During this time it displays a `syncing` spinner

Splash screen displays a list of proposals: title; for & against; status.

Have a `+` button to create a new proposal.

On a wide screen also display a panel that has info about the DAO and about the user:
- Current voting power (behind a "Connect wallet" interface like in register-ui)
- Current total DAO voting power
- Contract addresses (DAO contract; token registry contract; hypermap contract; HYPR token contract; gHYPR token contract)
- Number of nodes in offchain committee - and number reporting online

#### New proposal screen

- Create a new proposal: title, description, other relevant fields

#### Existing proposal screen

- Display status of proposal as a point in a timeline with other relevant items such as
  - Draft created
  - Published onchain (link to tx)
  - Start of voting period (with block number & datetime)
  - End of voting period
  - Queueing and execution
- Display for & against votes
- Can view who voted for which side
- Can see how close to quorum
- Show description
- Show tree-chat (chat with hierarchy of replies); sortable by time, upvote/downvotes, reply depth

#### DAO voting power breakdown screen

- A plot showing user's voting power vs time (default minimzed; show it when clickign into the current voting power)
- Pie chart showing top 6 voting power wallets (and "Other" = 7th piece of pie)
- A plot showing DAO's voting power vs time (total and top 3 wallets by default; can increase the number of top wallets to show or search wallet/node to display that)

#### Forwarding committee screen

- Clicking into forwarding committee shows who nodes are

### Storing and distributing information offchain

- Information is stored on each participating node
- Each participating node indexes the chain in a manner analogous to the hypermap-cacher example
- There is a committee of nodes that maintain the offchain data for the DAO
- Committee nodes are in consensus with each other using CRDT
- Committee nodes serve the current offchain state to other nodes that ask for it over the hyperware network (i.e. p2p)
- Other nodes can ask for a ping (of most recent state hash) when the state changes; this functions like a subscription
- Other nodes will additionally do a keepalive every hour; if the keepalive is not responded to they try another committee node until they get an Ack with the most recent state hash
- Committee nodes serve the entire state or pieces of state at request
- State includes:
  - Committee members
  - Proposal drafts
  - Discussion of proposals
- Any node can join the committe upon request (but have a conditional function here that is the `gatekeeper` that, for now, always returns `true`, but could, in future, add requirements for joining)
